 ...
 * @version 2.02 - implemented generic API in PKMT tool 2019-02-05
 */
...
public class Dictionary implements DictionaryInterface {
	...
	private DictEntry <String,String>[] entry;							// The actual dictionary as an array
	...
	public Dictionary(){
		...
		entry = new DictEntry[maxNumberOfEntries];		// Allocate the array
		word = null;
	}
	...
	public void addEntry(String w, String d){	
		...
		if (entry.length > numEntries + 1) {
			...
			if (w.compareTo(largestWord) > 0) {
				...
				entry[numEntries++] = new  DictEntry  <String,String>	(w, d);			// largest word
			} else {
				...
				entry[ndx] = new DictEntry <String,String>(w, d);
			}
		}
		else
	 ...
	 */
	public DictEntry <String,String> findNextEntry(){
		...
		while (currentIndex < numEntries-1) {	// While there are entries left to search
			DictEntry <String,String> de = entry[++currentIndex];	// Fetch the entry, convert it to upper case
	...
	public String findAll(String target){
		...
		DictEntry <String,String> aMatch = findNextEntry();		// Try to fetch the next entry
	 ...
	 */
	public DictEntry <String,String> getDictEntry(int ndx){		// Check the index to see if it is in range
	 ...
	 * @param <T,DictEntry> To setup the object of DictEntry
	 * @param index
	 * @param d Entry
	 */
	public <T> void setDictEntry(T index, DictEntry <String,String> d) {
		entry[(int) index]=d;
	}
	/*****
	 * Get Index of Entry
	 ...
	 * @return The Index
	 */
	public int getIndex(DictEntry <String,String> d) {
		List<DictEntry <String,String>> theList=Arrays.asList(entry);
		return theList.indexOf(d);
	}
